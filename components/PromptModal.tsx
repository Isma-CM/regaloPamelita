...
export const PromptModal: React.FC<PromptModalProps> = ({ isOpen, mode, onClose, onSubmit }) => {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      setError('¡La magia falló! Intenta de nuevo.');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className={`bg-white rounded-3xl shadow-2xl w-full max-w-lg flex flex-col border-4 ${isCreate ? 'border-sky-100' : 'border-amber-100'} animate-in fade-in zoom-in duration-200 scale-95 sm:scale-100 overflow-hidden`}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
                <h2 className="text-xl font-extrabold text-slate-800">
                    {isCreate ? 'Nueva Creación' : 'Reconstruir'}
                </h2>
        ...
        <div className="p-6 bg-white">
          <p className="text-slate-600 font-semibold mb-4">
            {isCreate 
                ? "¿Qué nueva creación deberíamos construir?" 
                : "¿Cómo deberíamos reconstruir los vóxeles?"}
          </p>
          ...
          <form onSubmit={handleSubmit}>
            <textarea 
              ...
              placeholder={isCreate 
                ? "ej. Un castillo medieval, un robot gigante, una canasta de frutas..." 
                : "ej. Conviértelo en un auto, haz una pirámide, construye una carita feliz..."}
              disabled={isLoading}
