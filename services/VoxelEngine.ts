...
export class VoxelEngine {
  ...
  public loadInitialModel(data: VoxelData[], startDismantled: boolean = false) {
    this.createVoxels(data, startDismantled);
  ...
  private createVoxels(data: VoxelData[], startDismantled: boolean) {
    // Clear existing
    ...
    this.voxels = data.map((v, i) => {
        ...
        // If starting dismantled, randomize positions immediately
        if (startDismantled) {
            x += (Math.random() - 0.5) * 30;
            y = CONFIG.FLOOR_Y + 0.5 + Math.random() * 5; // Start near floor
            z += (Math.random() - 0.5) * 30;
            
            // Give them some initial random velocity as if they just fell
            vx = (Math.random() - 0.5) * 0.8;
            vy = Math.random() * 0.5;
            vz = (Math.random() - 0.5) * 0.8;
            rvx = (Math.random() - 0.5) * 0.2;
            rvy = (Math.random() - 0.5) * 0.2;
            rvz = (Math.random() - 0.5) * 0.2;
        }

        return {
            id: i,
            x, y, z, color: c,
            vx, vy, vz, rx: 0, ry: 0, rz: 0,
            rvx, rvy, rvz
        };
    });
